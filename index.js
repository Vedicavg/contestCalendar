...
function updateProfileName() {
  chrome.storage.local.get(['user_name', 'user_email'], (data) => {
    ...
    if (profileName) profileName.textContent = `Hi! ${data.user_name || ''}`;
    if (profileEmail) profileEmail.textContent = data.user_email || '';
  });
}
...
async function getActiveContests() {
  ...
  chrome.storage.local.get('access_token', async (data) => {
    ...
    if (token) {
      const calendarEvents = await fetch('https://www.googleapis.com/calendar/v3/calendars/primary/events', {
        headers: { Authorization: `Bearer ${token}` }
      });
      const calendarData = await calendarEvents.json();
      if (calendarData.items) {
        existingTitles = calendarData.items.map(ev => ev.summary);
      }
    }
    ...
    activeContests.forEach(contest => {
      ...
      const startTime = new Date(contest.startTimeSeconds * 1000).toLocaleString('en-IN', options);
      const duration = `${contest.durationSeconds / 3600} hours`;
      const isMarked = existingTitles.includes(name);
      ...
      contestElement.innerHTML = `
        ...
        <button 
          class="add-to-calendar ${isMarked ? 'already-added' : ''}" 
          data-id="${contest.id}" 
          data-name="${name}" 
          data-start="${startTime}" 
          data-duration="${duration}">
          ${isMarked ? '✅ Already Added' : 'Add to Calendar'}
        </button>
      `;
...
function addToCalendar(contest) {
  chrome.storage.local.get('access_token', async (data) => {
    const token = data.access_token;
    if (!token) {
      alert('Please login to add the contest to your Google Calendar.');
      return;
    }
    ...
    if (createResponse.ok) {
      ...
      getActiveContests(); // refresh after adding
    } else {
      alert('Failed to add event. Try logging out and back in.');
      console.error('Event create error:', result);
    }
  });
}
  ...
  chrome.storage.local.get('access_token', (data) => {
    if (data.access_token) {
      chrome.storage.local.remove(['access_token', 'user_name', 'user_email'], () => {
        updateLoginButton();
        updateProfileName();
        getActiveContests(); // refresh contests
      });
    } else {
